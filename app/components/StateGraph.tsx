...
export default function StateGraph({ 
  ...
  // Generate edges
  const edges: { from: number; to: number; symbol: string; x1: number; y1: number; x2: number; y2: number }[] = [];
  ...
  const getArrowPoints = (x1: number, y1: number, x2: number, y2: number, nodeRadius: number) => {
    const dx = x2 - x1;
    const dy = y2 - y1;
        ...
        >
          ...
          {edges.map((edge, idx) => {
            const { startX, startY, endX, endY } = getArrowPoints(
              edge.x1, edge.y1, edge.x2, edge.y2, nodeRadius
            );
            ...
            return (
              <g key={idx} className={`edge ${isHighlighted ? 'highlighted' : ''}`}>
                <path
          ...
          {selfLoops.map((loop, idx) => {
            ...
            return (
              <g key={`loop-${idx}`} className={`edge ${isHighlighted ? 'highlighted' : ''}`}>
                <path
            ...
            return (
              ...
              >
                ...
                <circle
                  ...
                  cy={pos.y}
                  r={nodeRadius}
                  className={`node-circle ${isHighlighted ? 'highlighted' : ''} ${hasAccept ? 'accept' : ''}`}
                />
