        ...
        {states.map((state) => (
          <div
            ...
            className={`state-card ${
              highlightedState === state.id ? "highlighted" : ""
            } ${
              highlightedTransition?.from === state.id ||
              highlightedTransition?.to === state.id
                ? "transition-active"
                : ""
            }`}
          >
                        ...
                        <span className="item-rhs">
                          {before.map((s, i) => (
                            <span
                              key={`b${i}`}
                              className={
                                isTerminal(s) ? "terminal" : "non-terminal"
                              }
                            >
                              {s}
                            </span>
                ...
                {state.transitions.size > 0 && (
                  <div className="transitions">
                    ...
                    <div className="transitions-list">
                      {Array.from(state.transitions.entries()).map(
                        ([symbol, targetState]) => (
                          <div
                            key={symbol}
                          ...
                          >
                            <span
                              className={
                                isTerminal(symbol) ? "terminal" : "non-terminal"
                              }
                            >
                              {symbol}
                            </span>
